...
import {
  Activity, ArrowUpDown, Check, ChevronDown, Copy, Database, Edit3, Eye, FileQuestion,
...
type ModalDropdown = "category" | "difficulty" | null;
...
type TouchedState = { question: boolean; a: boolean; b: boolean; c: boolean; d: boolean };
...
const EMPTY_TOUCHED: TouchedState = { question: false, a: false, b: false, c: false, d: false };
...
export default function QuestionBankPage() {
  ...
  const [modalDropdown, setModalDropdown] = useState<ModalDropdown>(null);
  const [categoryDropdownDirection, setCategoryDropdownDirection] = useState<DropdownDirection>("down");
  ...
  useEffect(() => {
    ...
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (modalDropdown) setModalDropdown(null);
      else closeModal();
    }
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [modalOpen, modalDropdown]);
  ...
  function resetModalState() { setTouched(EMPTY_TOUCHED); setSubmitAttempted(false); setModalDropdown(null); setCategoryDropdownDirection("down"); }
  function openCreate() { setEditingId(null); setForm(EMPTY_FORM); resetModalState(); setModalOpen(true); }
  function openEdit(question: BankQuestion) { setEditingId(question.id); setForm({ text: question.text, choice_a: question.choice_a, choice_b: question.choice_b, choice_c: question.choice_c ?? "", choice_d: question.choice_d ?? "", correct_choice: question.correct_choice, category_id: question.category_id ?? "", difficulty: question.difficulty, duration_seconds: question.duration_seconds, is_active: question.is_active }); resetModalState(); setModalOpen(true); }
  function closeModal() { setModalOpen(false); setModalDropdown(null); }
  function markTouched(key: keyof TouchedState) { setTouched((current) => ({ ...current, [key]: true })); }
  function toggleCategoryDropdown(event: React.MouseEvent<HTMLButtonElement>) {
    if (modalDropdown === "category") { setModalDropdown(null); return; }
    const rect = event.currentTarget.getBoundingClientRect();
    const roomBelow = window.innerHeight - rect.bottom;
    const roomAbove = rect.top;
    setCategoryDropdownDirection(roomBelow < 235 && roomAbove > roomBelow ? "up" : "down");
    setModalDropdown("category");
  }

  const trimmedQuestion = form.text.trim();
  const trimmedA = form.choice_a.trim();
  const trimmedB = form.choice_b.trim();
  const trimmedC = form.choice_c.trim();
  const trimmedD = form.choice_d.trim();
  const selectedAnswerFilled = form.correct_choice === "a" ? Boolean(trimmedA) : form.correct_choice === "b" ? Boolean(trimmedB) : form.correct_choice === "c" ? Boolean(trimmedC) : Boolean(trimmedD);
  const formValid = Boolean(trimmedQuestion && trimmedA && trimmedB && selectedAnswerFilled);
  const showQuestionError = (touched.question || submitAttempted) && !trimmedQuestion;
  const showChoiceError = (key: ChoiceKey, value: string, required: boolean) => (touched[key] || submitAttempted) && ((required && !value.trim()) || (form.correct_choice === key && !value.trim()));

  async function saveQuestion() {
    setSubmitAttempted(true);
    if (!formValid) return;
    const text = trimmedQuestion, a = trimmedA, b = trimmedB, c = trimmedC, d = trimmedD;
    setSaving(true);
    ...
    closeModal(); notify(editingId ? "Question mise à jour." : "Question ajoutée à la banque."); await loadData();
  }
  ...
  const selectedCategory = form.category_id ? categoryMap.get(form.category_id) ?? null : null;
  ...
  return <div className="p-5 lg:p-8 flex flex-col gap-5 lg:gap-6">
    ...
    {modalOpen && <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) closeModal(); }}><div className="w-full max-w-[760px] rounded-2xl border border-auth-border bg-auth-panel shadow-2xl overflow-visible"><div className="px-5 py-4 border-b border-auth-border flex items-center justify-between"><div><h2 className="text-base font-bold text-auth-text">{editingId ? "Modifier la question" : "Nouvelle question"}</h2><p className="text-[10px] text-auth-muted mt-0.5">Enregistrée dans la banque réutilisable</p></div><button onClick={closeModal} className="w-8 h-8 rounded-lg flex items-center justify-center text-auth-muted hover:text-auth-text hover:bg-white/5 transition"><X size={18} /></button></div><div className={`p-5 space-y-4 ${modalDropdown ? "overflow-visible" : "max-h-[72vh] overflow-y-auto overscroll-contain [scrollbar-width:thin] [scrollbar-color:#343A50_transparent] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#343A50]"}`} onMouseDown={(e) => { const target = e.target as HTMLElement; if (!target.closest('[data-modal-dropdown]')) setModalDropdown(null); }}>
      <div><div className="flex items-center justify-between gap-3"><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Question *</label><span className={`text-[9px] ${form.text.length > 220 ? "text-amber-400" : "text-auth-mutedDim"}`}>{form.text.length}/250</span></div><textarea maxLength={250} value={form.text} onChange={(e) => setForm((f) => ({ ...f, text: e.target.value }))} onBlur={() => markTouched("question")} rows={3} placeholder="Saisissez votre question..." className={`mt-1.5 w-full rounded-lg border bg-auth-bg p-3 text-sm text-auth-text outline-none placeholder:text-auth-mutedDim transition ${showQuestionError ? "border-red-500/45 focus:border-red-400" : "border-auth-border focus:border-auth-blue"}`} />{showQuestionError && <p className="mt-1.5 text-[10px] text-red-400">Saisissez une question avant de l’ajouter à la banque.</p>}</div>
      <div className="rounded-xl border border-auth-border bg-auth-bg/50 p-3.5"><div className="flex flex-wrap items-center justify-between gap-3 mb-3"><div><p className="text-[10px] uppercase font-bold tracking-[0.12em] text-auth-mutedDim">Bonne réponse</p><p className="text-[10px] text-auth-muted mt-0.5">Sélectionnez la lettre correspondant à la bonne réponse.</p></div><div className="flex items-center gap-1.5">{(["a","b","c","d"] as ChoiceKey[]).map((key) => { const value = form[`choice_${key}` as keyof QuestionForm] as string; const disabled = (key === "c" || key === "d") && !value.trim(); const active = form.correct_choice === key; return <button key={key} type="button" disabled={disabled} onClick={() => setForm((f) => ({ ...f, correct_choice: key }))} className={`w-9 h-9 rounded-lg border text-[11px] font-bold uppercase transition ${active ? "border-green-500/50 bg-green-500/15 text-green-300 shadow-[0_0_0_1px_rgba(34,197,94,0.08)]" : disabled ? "border-auth-border bg-black/10 text-auth-mutedDim opacity-45 cursor-not-allowed" : "border-auth-border bg-auth-panel text-auth-muted hover:text-auth-text hover:border-white/20"}`}>{key}</button>; })}</div></div>
        <div className="grid sm:grid-cols-2 gap-3">{(["a","b","c","d"] as ChoiceKey[]).map((key) => { const required = key === "a" || key === "b"; const value = form[`choice_${key}` as keyof QuestionForm] as string; const correct = form.correct_choice === key; const hasError = showChoiceError(key, value, required); return <div key={key}><div className="flex items-center justify-between"><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Réponse {key.toUpperCase()} {required ? "*" : ""}</label>{!required && <span className="text-[9px] text-auth-mutedDim">Optionnel</span>}</div><div className={`mt-1.5 flex gap-2 rounded-lg ${correct && !hasError ? "ring-1 ring-green-500/35" : ""}`}><div className={`w-9 shrink-0 rounded-lg border flex items-center justify-center text-[10px] font-bold uppercase ${hasError ? "border-red-500/35 bg-red-500/[0.06] text-red-400" : correct ? "border-green-500/40 bg-green-500/10 text-green-400" : "border-auth-border bg-auth-bg text-auth-muted"}`}>{key}</div><input value={value} onChange={(e) => setForm((f) => ({ ...f, [`choice_${key}`]: e.target.value }))} onBlur={() => markTouched(key)} placeholder={required ? "Réponse obligatoire" : "Réponse optionnelle"} className={`flex-1 min-w-0 rounded-lg border bg-auth-bg px-3 py-2.5 text-xs text-auth-text outline-none placeholder:text-auth-mutedDim transition ${hasError ? "border-red-500/45 focus:border-red-400" : "border-auth-border focus:border-auth-blue"}`} /></div>{hasError && <p className="mt-1.5 text-[9px] text-red-400">{correct ? "La bonne réponse doit contenir du texte." : "Cette réponse est obligatoire."}</p>}</div>; })}</div>
      </div>
      <div className="grid sm:grid-cols-2 gap-3 relative z-20">
        <div className="relative" data-modal-dropdown><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Catégorie</label><button type="button" onClick={toggleCategoryDropdown} className={`mt-1.5 w-full h-10 rounded-lg border bg-auth-bg px-3 flex items-center justify-between gap-3 text-xs transition ${modalDropdown === "category" ? "border-auth-blue/70 ring-1 ring-auth-blue/15" : "border-auth-border hover:border-white/15"}`}><span className="flex items-center gap-2 min-w-0"><span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: selectedCategory?.color ?? "#6B7086" }} /><span className={`truncate ${selectedCategory ? "text-auth-text" : "text-auth-muted"}`}>{selectedCategory?.name ?? "Sans catégorie"}</span></span><ChevronDown size={14} className={`text-auth-mutedDim transition-transform ${modalDropdown === "category" ? "rotate-180" : ""}`} /></button>{modalDropdown === "category" && <div className={`absolute left-0 right-0 z-50 rounded-xl border border-auth-border bg-[#0C0E17] shadow-2xl p-1.5 max-h-[196px] overflow-y-auto overscroll-contain [scrollbar-width:thin] [scrollbar-color:#343A50_transparent] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#343A50] ${categoryDropdownDirection === "up" ? "bottom-[46px]" : "top-[64px]"}`}><button type="button" onClick={() => { setForm((f) => ({ ...f, category_id: "" })); setModalDropdown(null); }} className={`w-full rounded-lg px-3 py-2.5 flex items-center gap-2 text-xs text-left transition ${!form.category_id ? "bg-auth-blue/12 text-auth-text" : "text-auth-muted hover:bg-white/5 hover:text-auth-text"}`}><span className="w-2.5 h-2.5 rounded-full bg-[#6B7086]" /><span className="flex-1">Sans catégorie</span>{!form.category_id && <Check size={13} className="text-auth-blue" />}</button>{categories.filter((c) => c.is_active).map((category) => <button key={category.id} type="button" onClick={() => { setForm((f) => ({ ...f, category_id: category.id })); setModalDropdown(null); }} className={`w-full rounded-lg px-3 py-2.5 flex items-center gap-2 text-xs text-left transition ${form.category_id === category.id ? "bg-auth-blue/12 text-auth-text" : "text-auth-muted hover:bg-white/5 hover:text-auth-text"}`}><span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: category.color }} /><span className="flex-1 truncate">{category.name}</span>{form.category_id === category.id && <Check size={13} className="text-auth-blue" />}</button>)}</div>}</div>
        <div className="relative" data-modal-dropdown><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Difficulté</label><button type="button" onClick={() => setModalDropdown((current) => current === "difficulty" ? null : "difficulty")} className={`mt-1.5 w-full h-10 rounded-lg border bg-auth-bg px-3 flex items-center justify-between gap-3 text-xs transition ${modalDropdown === "difficulty" ? "border-auth-blue/70 ring-1 ring-auth-blue/15" : "border-auth-border hover:border-white/15"}`}><span className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full" style={{ background: DIFFICULTY_COLOR[form.difficulty] }} /><span className="text-auth-text">{DIFFICULTY_LABEL[form.difficulty]}</span></span><ChevronDown size={14} className={`text-auth-mutedDim transition-transform ${modalDropdown === "difficulty" ? "rotate-180" : ""}`} /></button>{modalDropdown === "difficulty" && <div className="absolute left-0 right-0 top-[64px] z-50 rounded-xl border border-auth-border bg-[#0C0E17] shadow-2xl p-1.5">{(["easy","medium","hard"] as Difficulty[]).map((difficulty) => <button key={difficulty} type="button" onClick={() => { setForm((f) => ({ ...f, difficulty })); setModalDropdown(null); }} className={`w-full rounded-lg px-3 py-2.5 flex items-center gap-2 text-xs text-left transition ${form.difficulty === difficulty ? "bg-auth-blue/12 text-auth-text" : "text-auth-muted hover:bg-white/5 hover:text-auth-text"}`}><span className="w-2.5 h-2.5 rounded-full" style={{ background: DIFFICULTY_COLOR[difficulty] }} /><span className="flex-1">{DIFFICULTY_LABEL[difficulty]}</span>{form.difficulty === difficulty && <Check size={13} className="text-auth-blue" />}</button>)}</div>}</div>
      </div>
      <div><div className="flex items-center justify-between gap-3"><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Durée</label><span className="text-[10px] font-semibold text-auth-text">{form.duration_seconds} s</span></div><div className="mt-1.5 grid grid-cols-5 gap-2">{DURATION_OPTIONS.map((seconds) => <button key={seconds} type="button" onClick={() => setForm((f) => ({ ...f, duration_seconds: seconds }))} className={`rounded-lg border py-2.5 text-[10px] font-semibold transition ${form.duration_seconds === seconds ? "border-auth-blue/50 bg-auth-blue/10 text-auth-text" : "border-auth-border bg-auth-bg text-auth-muted hover:text-auth-text"}`}>{seconds} s</button>)}</div></div>
      <label className="flex items-center gap-3 rounded-lg border border-auth-border bg-auth-bg p-3 cursor-pointer hover:border-white/15 transition"><input type="checkbox" checked={form.is_active} onChange={(e) => setForm((f) => ({ ...f, is_active: e.target.checked }))} /><div><p className="text-xs font-semibold text-auth-text">Question active</p><p className="text-[10px] text-auth-muted">Disponible pour les prochaines sessions.</p></div></label>
      {submitAttempted && !formValid && <div className="rounded-lg border border-red-500/20 bg-red-500/[0.045] px-3 py-2.5 text-[10px] text-red-300 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-red-400 shrink-0" />Vérifiez uniquement les champs signalés en rouge avant l’enregistrement.</div>}
    </div><div className="px-5 py-4 border-t border-auth-border flex items-center justify-between gap-3"><p className="text-[10px] text-auth-muted hidden sm:block">Les champs marqués d’un * sont obligatoires.</p><div className="flex justify-end gap-2 ml-auto"><button onClick={closeModal} className="rounded-lg border border-auth-border px-4 py-2.5 text-xs font-semibold text-auth-muted hover:text-auth-text hover:bg-white/5 transition">Annuler</button><button onClick={saveQuestion} disabled={saving} aria-disabled={!formValid} className={`rounded-lg border px-4 py-2.5 text-xs font-semibold text-white transition ${!formValid ? "opacity-60 border-white/10 shadow-none saturate-75" : "border-transparent hover:brightness-110 shadow-[0_8px_24px_rgba(155,77,255,0.16)]"} disabled:opacity-35 disabled:cursor-not-allowed`} style={{ background: "linear-gradient(135deg,#4C6FFF,#9B4DFF,#FF3D8E)" }}>{saving ? "Enregistrement…" : editingId ? "Enregistrer" : "Ajouter à la banque"}</button></div></div></div></div>}
  </div>;
}
