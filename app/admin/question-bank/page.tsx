...
export default function QuestionBankPage() {
  ...
  function resetModalState() { setTouched(EMPTY_TOUCHED); setSubmitAttempted(false); setModalDropdown(null); }
  ...
  return <div className="p-5 lg:p-8 flex flex-col gap-5 lg:gap-6">
    ...
    {modalOpen && <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) closeModal(); }}><div className="w-full max-w-2xl rounded-2xl border border-auth-border bg-auth-panel shadow-2xl overflow-visible"><div className="px-5 py-4 border-b border-auth-border flex items-center justify-between"><div><h2 className="text-base font-bold text-auth-text">{editingId ? "Modifier la question" : "Nouvelle question"}</h2><p className="text-[10px] text-auth-muted mt-0.5">Enregistrée dans la banque réutilisable</p></div><button onClick={closeModal} className="w-8 h-8 rounded-lg flex items-center justify-center text-auth-muted hover:text-auth-text hover:bg-white/5 transition"><X size={18} /></button></div><div className="p-5 space-y-4 max-h-[72vh] overflow-y-auto overflow-x-visible">
