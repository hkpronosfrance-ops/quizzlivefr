  ...
  return <div className="p-5 lg:p-8 flex flex-col gap-5 lg:gap-6">
    ...
    {modalOpen && <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"><div className="w-full max-w-2xl rounded-2xl border border-auth-border bg-auth-panel shadow-2xl overflow-hidden"><div className="px-5 py-4 border-b border-auth-border flex items-center justify-between"><div><h2 className="text-base font-bold text-auth-text">{editingId ? "Modifier la question" : "Nouvelle question"}</h2><p className="text-[10px] text-auth-muted mt-0.5">Enregistrée dans la banque réutilisable</p></div><button onClick={() => setModalOpen(false)} className="text-auth-muted hover:text-auth-text"><X size={18} /></button></div><div className="p-5 space-y-4 max-h-[72vh] overflow-y-auto">
      ...
      <div className="rounded-xl border border-auth-border bg-auth-bg/50 p-3.5"><div className="flex flex-wrap items-center justify-between gap-3 mb-3"><div><p className="text-[10px] uppercase font-bold tracking-[0.12em] text-auth-mutedDim">Bonne réponse</p><p className="text-[10px] text-auth-muted mt-0.5">Sélectionnez la lettre correspondant à la bonne réponse.</p></div><div className="flex items-center gap-1.5">{(["a","b","c","d"] as ChoiceKey[]).map((key) => { const value = form[`choice_${key}` as keyof QuestionForm] as string; const disabled = (key === "c" || key === "d") && !value.trim(); const active = form.correct_choice === key; return <button key={key} type="button" disabled={disabled} onClick={() => setForm((f) => ({ ...f, correct_choice: key }))} className={`w-9 h-9 rounded-lg border text-[11px] font-bold uppercase transition ${active ? "border-green-500/50 bg-green-500/15 text-green-300 shadow-[0_0_0_1px_rgba(34,197,94,0.08)]" : disabled ? "border-auth-border bg-black/10 text-auth-mutedDim opacity-45 cursor-not-allowed" : "border-auth-border bg-auth-panel text-auth-muted hover:text-auth-text hover:border-white/20"}`}>{key}</button>; })}</div></div>
        <div className="grid sm:grid-cols-2 gap-3">{(["a","b","c","d"] as ChoiceKey[]).map((key) => { const required = key === "a" || key === "b"; const value = form[`choice_${key}` as keyof QuestionForm] as string; const correct = form.correct_choice === key; return <div key={key}><div className="flex items-center justify-between"><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Réponse {key.toUpperCase()} {required ? "*" : ""}</label>{!required && <span className="text-[9px] text-auth-mutedDim">Optionnel</span>}</div><div className={`mt-1.5 flex gap-2 rounded-lg ${correct ? "ring-1 ring-green-500/35" : ""}`}><div className={`w-9 shrink-0 rounded-lg border flex items-center justify-center text-[10px] font-bold uppercase ${correct ? "border-green-500/40 bg-green-500/10 text-green-400" : "border-auth-border bg-auth-bg text-auth-muted"}`}>{key}</div><input value={value} onChange={(e) => setForm((f) => ({ ...f, [`choice_${key}`]: e.target.value }))} placeholder={required ? "Réponse obligatoire" : "Réponse optionnelle"} className="flex-1 min-w-0 rounded-lg border border-auth-border bg-auth-bg px-3 py-2.5 text-xs text-auth-text outline-none focus:border-auth-blue placeholder:text-auth-mutedDim" /></div>{required && !value.trim() && <p className="mt-1.5 text-[9px] text-auth-muted">Cette réponse est obligatoire.</p>}{correct && !value.trim() && <p className="mt-1.5 text-[9px] text-amber-400">La bonne réponse sélectionnée doit contenir du texte.</p>}</div>; })}</div>
      </div>
      <div className="grid sm:grid-cols-2 gap-3"><div><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Catégorie</label><select value={form.category_id} onChange={(e) => setForm((f) => ({ ...f, category_id: e.target.value }))} className="mt-1.5 w-full bg-auth-bg border border-auth-border rounded-lg px-3 py-2.5 text-xs text-auth-text"><option value="">Sans catégorie</option>{categories.filter((c) => c.is_active).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div><label className="text-[10px] uppercase font-bold text-auth-mutedDim">Difficulté</label><select value={form.difficulty} onChange={(e) => setForm((f) => ({ ...f, difficulty: e.target.value as Difficulty }))} className="mt-1.5 w-full bg-auth-bg border border-auth-border rounded-lg px-3 py-2.5 text-xs text-auth-text"><option value="easy">Facile</option><option value="medium">Moyen</option><option value="hard">Difficile</option></select></div></div>
      ...
      <label className="flex items-center gap-3 rounded-lg border border-auth-border bg-auth-bg p-3 cursor-pointer"><input type="checkbox" checked={form.is_active} onChange={(e) => setForm((f) => ({ ...f, is_active: e.target.checked }))} /><div><p className="text-xs font-semibold text-auth-text">Question active</p><p className="text-[10px] text-auth-muted">Disponible pour les prochaines sessions.</p></div></label>
      {!formValid && <div className="rounded-lg border border-amber-500/20 bg-amber-500/[0.05] px-3 py-2.5 text-[10px] text-amber-300">Complétez la question, les réponses A et B, puis choisissez une bonne réponse contenant du texte.</div>}
    </div><div className="px-5 py-4 border-t border-auth-border flex justify-end gap-2"><button onClick={() => setModalOpen(false)} className="rounded-lg border border-auth-border px-4 py-2.5 text-xs font-semibold text-auth-muted hover:text-auth-text">Annuler</button><button onClick={saveQuestion} disabled={saving || !formValid} className="rounded-lg px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-35 disabled:cursor-not-allowed" style={{ background: "linear-gradient(135deg,#4C6FFF,#9B4DFF,#FF3D8E)" }}>{saving ? "Enregistrement…" : editingId ? "Enregistrer" : "Ajouter à la banque"}</button></div></div></div>}
  </div>;
}
